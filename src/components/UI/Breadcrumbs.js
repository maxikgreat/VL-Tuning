    ...
    return(
        <div className = "breadcrumbsContainer">
            <ul id="breadcrumb">
                ...
                {model ? <li><Link to = {`/production/${typeStuff}/${brand.replace("-","").toLowerCase()}/${model.ID}`}><span className="icon"></span><span className = "bread-text">{model.Name.substring(0, 20)} ...</span></Link></li> : null}
            </ul>
        </div>
    )
}
